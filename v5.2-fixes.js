...
// Bug 1: the year-8/9 market event must NOT be called the final market window.
// Only year 10 may use the true finalMarket event. Earlier appearances are converted
// into a separate mid/late-stage growth event with its own id, title, copy and choices.
  ...
  if(state.year<10){
    if(state.seen.includes('lateGrowthWindow')){
      ...
      return {
        ...
        opts:[
          {label:'A · 压缩低效业务',detail:'停掉回报低、售后重的部分，把人力和现金留给已经验证过的业务。',after:'你砍掉了两项看起来热闹、实际不断消耗时间的工作。收入没有立刻暴涨，但团队第一次感觉每天做的事情更集中。',{cash:900,business:-400,credit:2,risk:-2,stress:-3,flag:'late_review_efficiency'}},
          {label:'B · 回访老客户',detail:'不追新流量，集中联系过去几年真正复购过的人。',after:'你没有买新的流量，而是把时间花在老客户和旧合作方身上。订单增长不猛，却让几段快要断掉的关系重新活了起来。',{cash:-500,business:1500,credit:4,skill:1,flag:'late_review_clients'}},
          {label:'C · 试一个新细分品类',detail:'用小预算验证一个新方向，限制库存和试错规模。',after:'你只做了一个小批量测试。它没有改变全局，却给第九、十年的经营留下了一个新的可能性。',{cash:-1200,inventory:1500,skill:2,risk:2,flag:'late_review_product'}}
        ]
      };
    }
    ...
    return {
      ...
      opts:[
        {label:'A · 只放大已经验证过的爆款',detail:'集中补最熟悉的品类，不同时扩新品和新渠道。增长更集中，但库存与交付仍会明显承压。',after:'你没有把所有热度都接进来，只把预算压在过去两年最稳定的一条线上。销量明显上升，阿杰觉得你终于敢放大优势，苏岚则一直盯着发货上限，避免爆单把售后拖垮。',{cash:-4200,inventory:5200,business:2600,credit:2,risk:3,stress:3,flag:'late_growth_focus'}},
        {label:'B · 做老客户限定活动',detail:'不争平台最大流量，优先让过去成交过的人复购。增长上限较低，但回款、售后和需求更可控。',after:'活动没有冲上排行榜，却带回一批很久没联系的客户。订单量没有失控，团队还能把每个问题处理完。许宁认为这轮增长质量比数字更重要。',{cash:-1800,business:2300,credit:5,risk:-1,stress:-1,flag:'late_growth_clients'}},
        {label:'C · 放弃旺季扩张，修下一阶段基础',detail:'不追这轮增长，把现金和时间用于库存、流程、合同和团队分工。短期会错过销售高峰。',after:'群里不断有人晒大促成绩，你却把仓库和账重新理了一遍。这个季度的销售没有明显上升，但几项过去靠人记着的工作终于变成了稳定流程。',{cash:-900,skill:3,credit:2,risk:-3,stress:-4,flag:'late_growth_systems'}}
      ]
    };
  }

  // Year 10: this is the only true final market window.
  return {
    ...e,
    id:'finalMarket',
    title:'最后一次市场窗口',
    ...
    opts:[
      {label:'A · 参加最后一次大促，冲刺终点',detail:'只开放经过验证的商品，并设定库存、客服和发货上限。你仍然选择增长，但这次不允许无限加码。',after:'最后一轮订单迅速涌入。团队没有追求所有流量，而是在达到预设上限后主动关掉部分入口。你抓住了最后增长，也第一次知道什么时候必须停手。',{cash:-5200,inventory:6500,business:4800,credit:2,risk:5,stress:4,flag:'final_sprint'}},
      {label:'B · 收住规模，把成果变成现金',detail:'停止追逐新增流量，把库存、应收和临时项目逐项收口，让十年结局建立在真正能兑现的资产上。',after:'你没有再冲销量。团队花最后几个月清库存、催回款、结清临时合作。数字增长慢了，但账第一次真正有了“可以交卷”的感觉。',{cash:7200,inventory:-2400,business:-700,credit:4,risk:-5,stress:-5,flag:'final_close'}},
      {label:'C · 重排十年后的业务与生活',detail:'降低高压业务占比，保留稳定现金流，并重新谈清团队权限、投资边界和个人生活安排。',after:'最后几个月没有出现夸张的销售峰值。你和苏岚重新谈权限，与许宁把经营现金和风险资产彻底分开，也拒绝了两项继续透支生活的机会。十年结束时，你留下的不只是净资产，还有下一阶段可以继续执行的边界。',{cash:-1000,business:1800,credit:4,skill:2,risk:-4,stress:-7,flag:'final_rebalance'}}
    ]
  };
};

// Bug 2: make the four relationships accumulate in visibly different directions.
...
V.relation=(state,e,o)=>{
  ...
  // Choice-style bias: repeated A/B/C patterns no longer leave all four people in one band.
